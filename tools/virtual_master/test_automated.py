...
def test_automated():
    ...
    try:
        ...
        if success_count >= 3:
            ...
            print(
                f"❌ Communication cycles: only {success_count}/{total_cycles} successful"
            )
            return 1
        ...
        if product_data:
            print(
                f"✅ Index 0x0012 (Product Name): '{product_data.decode('ascii', errors='ignore')}'"
            )
        else:

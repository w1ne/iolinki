...
class TestSystemCommands(unittest.TestCase):
    ...
    def test_03_factory_restore_0x82(self):
        ...
        # Write a custom application tag
        test_tag = b'CustomTag123'
        write_result = self.master.write_isdu(index=0x0018, subindex=0x00, data=test_tag)
        ...
        self.assertEqual(readback, test_tag, "Application tag should match written value")
        
        # Send factory restore command
        ...
        # Verify application tag was reset (should be empty or default)
        readback_after = self.master.read_isdu(index=0x0018, subindex=0x00)
        self.assertNotEqual(readback_after, test_tag, "Application tag should be reset after factory restore")
        
        print(f"[PASS] Factory restore executed, tag reset from {test_tag} to {readback_after}")

    def test_04_restore_app_defaults_0x83(self):
        """Test system command 0x83: Restore Application Defaults"""
        print("\n[TEST] System Command 0x83: Restore Application Defaults")
        
        # Write 0x83 to index 0x0002

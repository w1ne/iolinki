...
class ISDUControlByte:
    ...
    def generate(start: bool, last: bool, seq: int) -> int:
        ...
        if start:
            cb |= ISDUControlByte.START
        if last:
            cb |= ISDUControlByte.LAST
        return cb
...
class MSequenceGenerator:
    ...
    def generate_type1(
        ...
        ck = calculate_checksum_type1(
            mc, ckt, pd, od, od2 if self.od_len == 2 else None
        )
...
class DeviceResponse:
    ...
    def __init__(self, data: bytes, od_len: int = 1):
        ...
        if self.valid and len(data) > 2:
            print(f"[DEBUG] Resp raw={data.hex()} od_len={od_len}")
        ...
        if self.valid:
            if len(data) == 2:
                ...
                for candidate_status in range(256):
                    if (
                        calculate_checksum_type0(candidate_status, self.od)
                        == self.checksum
                    ):

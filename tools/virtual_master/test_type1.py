...
def run_device_in_background(tty_path, m_seq_type=1, pd_len=2):
    ...
    try:
        # Start Device process with Type 1, PD Len 2
        proc = subprocess.Popen(
            [device_path, tty_path, str(m_seq_type), str(pd_len)]
        )
        print(f"[INFO] Device started (PID: {proc.pid}, Type={m_seq_type}, PD={pd_len})")
        time.sleep(0.5)
        return proc
...
def test_type1_communication():
    ...
    try:
        # Create Virtual Master configured for Type 1
        # m_seq_type=1 (Type 1_x), pd_in_len=2, pd_out_len=2
        ...
        for i, out_val in enumerate(test_data):
            ...
            # Host demo implements Echo + 1, but we see it in the NEXT cycle
            if prev_expected and response.payload == prev_expected:
                print(f"   ✅ Received PD_IN={response.payload.hex()} (matches previous cycle + 1)")
            elif prev_expected:
                print(f"   ⚠️ Data mismatch! Expected {prev_expected.hex()}, got {response.payload.hex()}")
        ...
        if vendor_name_bytes:
            ...
            if "iolinki" in vendor_name or "Acme" in vendor_name: # Default might be Acme or iolinki
        ...
        if master.write_isdu(0x18, 0, test_tag):
             ...
             if read_back == test_tag:
                 ...
                 # return 1 # Fail if write confirm fails
        else:

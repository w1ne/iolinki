...
class TestErrorInjectionConformance(unittest.TestCase):
    ...
    def test_07_crc_fallback_recovery(self):
        ...
        if hasattr(self.master, 'run_cycle_bad_crc'):
            ...
            time.sleep(0.2)  # Allow fallback state transition
        else:
        ...
        print(f"[INFO] Device recovered: Product Name = {response.decode('ascii', errors='ignore')}")
        ...
        for i in range(0, len(events_data), 3):
            if i + 2 < len(events_data):
                ...
                if code == 0x1801:
                    ...
                    break
        self.assertTrue(found_crc_event, "CRC error event (0x1801) should be present in Detailed Device Status")
        ...
        self.assertGreaterEqual(status_data[0], 1, "Device Status should be non-zero (reported error)")
        ...
        self.assertEqual(pop_code, 0x1801, f"Expected 0x1801 popped from Index 2, got 0x{pop_code:04X}")
        ...
        if events_data_after:
             ...
             for i in range(0, len(events_data_after), 3):
                 if i + 2 < len(events_data_after):
                     if (events_data_after[i+1] << 8) | events_data_after[i+2] == 0x1801:
                         found_again = True
             self.assertFalse(found_again, "Popped event should no longer be in Detailed Device Status")

        print("[PASS] Device recovered after CRC fallback, full functionality and event popping verified")

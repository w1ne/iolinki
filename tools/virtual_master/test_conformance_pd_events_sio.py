...
from virtual_master.protocol import DeviceResponse, MSequenceType
...
def launch_device(master):
    ...
    return subprocess.Popen([device_bin, device_tty], 
                          stdout=sys.stdout, 
                          stderr=sys.stderr,
...
def test_pd_toggle_bit():
    ...
    process = subprocess.Popen([device_bin, device_tty, "1", "2"], 
                              stdout=sys.stdout, 
                              stderr=sys.stderr,
                              preexec_fn=os.setsid)

...
def main():
    ...
    process = subprocess.Popen(
        [demo_bin, master.get_device_tty(), "1", "2"],
    ...
    try:
        ...
        if hasattr(master, "run_cycle_bad_crc"):
            ...
            print(
                "   ⚠️  Virtual Master doesn't support CRC injection, skipping inject."
            )
            return
        ...
        if status and status[0] > 0:
            ...
            print(
                f"   ❌ Device Status: {status[0] if status else 'None'} (Expected > 0)"
            )
            sys.exit(1)

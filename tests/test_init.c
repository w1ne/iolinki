...
static int local_mock_phy_init(void) { return (int)mock(); }
static void local_mock_phy_set_mode(iolink_phy_mode_t mode) { (void)mode; }
static void local_mock_phy_set_baudrate(iolink_baudrate_t baudrate) { (void)baudrate; }
static int local_mock_phy_send(const uint8_t *data, size_t len) { (void)data; (void)len; return 0; }
static int local_mock_phy_recv_byte(uint8_t *byte) { (void)byte; return 0; }

static const iolink_phy_api_t local_phy_mock = {
    .init = local_mock_phy_init,
